"""qwen3-32B para a extração/comparação de regras — autocontido, config própria
(thinking on, determinístico). Mesmo padrão de extract_rules/llm.py: reusa o
TransformersModelClient do repo e carrega o modelo UMA vez (lru_cache).
...
def generate_batch(llm: ModelClient, prompts: list[str], *, chunk: int) -> list[str]:
    """Gera em blocos de tamanho fixo, com barra de progresso. Prompts vazios
    são pulados."""
    idx = [i for i, p in enumerate(prompts) if p and p.strip()]
    out = [""] * len(prompts)
    with tqdm(total=len(idx), desc="Comparando regras", unit="regra") as bar:
